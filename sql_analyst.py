...
SELECT
  ...
  CASE final_stage
    ...
    WHEN 'completed' THEN 'Disbursement'
    WHEN 'disbursed' THEN 'Disbursement'
  END AS funnel_stage
...
- final_stage: traffic | submitted | approved | completed/disbursed (furthest funnel stage reached)
- funnel_stage: Traffic | Submission | Approval | Disbursement
- stage_rank: 1..4 (traffic=1, submitted=2, approved=3, completed/disbursed=4)
- drop_transition: traffic_to_submission | submission_to_approval | approval_to_disbursement | blank when disbursed
- drop_reason: blank only when disbursed. It explains why the user/entity failed to reach the next stage.
Counting rules:
- traffic/users = COUNT(*)
- submitted = COUNT(stage_rank >= 2)
- approved = COUNT(stage_rank >= 3)
- disbursed = COUNT(stage_rank = 4)
...
def schema_guide_markdown() -> str:
    ...
    return (
        ...
        "**Good prompts:** `/funnel show daily volume in May`, `/funnel break May approval drop down by reason`, "
        "`/funnel show May by product`, `/funnel show May by channel`, `/funnel why did approval drop?`, or "
        "`/funnel break May drop reasons by transition`. (`/query` still works as an advanced/audit alias.)"
    )
...
_STAGE_LOSSES = {
    ...
    "completion": {
        "label": "Approval -> Disbursement drop",
        "loss_key": "approval_to_disbursement",
        "failed": "stage_rank = 3",
...
def _amount_expr() -> str:
    return (
        ...
        "SUM(CASE WHEN stage_rank = 4 THEN potential_value_vnd ELSE 0 END) AS disbursement_amount_vnd"
    )
...
def _detect_drop_stage(q: str) -> str:
    ...
    if any(k in q for k in ["traffic", "submission drop", "submit drop", "not submit", "not submitted"]):
        return "submission"
    if any(k in q for k in ["disbursement", "disburse", "disbursed", "completion", "complete", "completed", "final outcome", "payout"]):
        return "completion"
    return "approval"
...
WITH scoped AS (
  ...
  SELECT
    CASE
      ...
      WHEN stage_rank = 3 THEN 'Approval -> Disbursement drop'
    END AS loss_stage,
    COALESCE(NULLIF(drop_reason, ''), 'unknown') AS drop_reason
  FROM scoped
  WHERE stage_rank < 4
