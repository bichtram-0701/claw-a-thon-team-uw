"""Funnel Agent / Funnel Agent — Team UW, Claw-a-thon 2026.
...
def _render_target_disbursement_answer(result: dict) -> str:
    ...
    lines = [
        ...
        "",
        f"For **{row.get('month')}**:",
        f"- Actual Disbursement volume: **{disb:,}**",
        f"- Actual Disbursement amount: **{amount:,.0f} VND**",
    ]
    ...
    lines += [
        "",
        "So if you mean the board-style count row, use **Disbursement (4)**. If you mean money, use **Disbursement Amount**.",
    ]
    return "\n".join(lines)
