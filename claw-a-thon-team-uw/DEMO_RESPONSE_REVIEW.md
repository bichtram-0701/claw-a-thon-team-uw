...
- Chat UI now describes a generic demo funnel: `Traffic -> Submission -> Approval -> Disbursement`.
...
- The old `break May down by drop reason` response counted all May rows and included disbursed rows as blank drop reasons.
...
- May 2026 now reconciles to `Traffic 800 -> Submission 216 -> Approval 24 -> Disbursement 23`.
...
Observed in `Funnel Agent - Team UW_v10(1).html`:
