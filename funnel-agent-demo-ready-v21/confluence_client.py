"""Confluence Cloud client for Funnel Agent.
...
def upsert_page(title: str, markdown_body: str) -> dict[str, Any]:
    ...
    payload = {
        ...
        "spaceId": sid,
        "body": {"representation": "storage", "value": markdown_to_storage(markdown_body)},
        "version": {"number": version_no, "message": "Updated by Funnel Agent weekly summary"},
    }
    with _client() as c:
...
def weekly_title(as_of: str | None = None) -> str:
    return "Weekly Funnel Agent Summary - " + (as_of or date.today().isoformat())
