...
| `completed` | Completed final outcome | 4 |
...
completion_rate = Completion / Approval
e2e_rate        = Completion / Traffic
```
...
| `potential_value_vnd` | Synthetic potential/completed value in VND |
| `final_stage` | Furthest stage reached: `traffic`, `submitted`, `approved`, `completed` |
| `drop_transition` | Failed transition for non-completed rows |
| `drop_reason` | Reason the user/entity did not reach the next stage; blank when completed |
...
| Month | Traffic | Submission | Approval | Completion |
|---|---:|---:|---:|---:|
...
|---|---|---|
| Completion timestamp mismatch | verification status-map alignment from the partner feed | reliable Completion timestamp reconciliation and final-outcome reporting |
