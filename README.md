...
Funnel Agent is a closed-loop execution intelligence agent for business funnels. It is designed for any recurring funnel owned by a team: marketing acquisition, product onboarding, merchant activation, sales pipeline, operations workflow, or a simple application funnel.
...
```text
Weekly Funnel Agent Summary - YYYY-MM-DD
```
...
- `data/funnel_synthetic.csv`: source-of-truth row-level fixture, one distinct synthetic user/entity per row. Traffic = all rows; Submission = `stage_rank >= 2`; Approval = `stage_rank >= 3`; Disbursement = `stage_rank = 4`.
