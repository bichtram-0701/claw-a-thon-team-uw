"""Jira Cloud REST client — Funnel Agent, Team UW.
...
def _blocker_context(summary: str | None, description: str | None) -> tuple[str | None, str | None]:
    ...
    if not blocked_by and ("disbursement timestamp mismatch" in lower or "disbursement timestamp mismatch" in lower):
        blocked_by = "verification status-map alignment from the partner feed"
    if not blocks and ("disbursement timestamp mismatch" in lower or "disbursement timestamp mismatch" in lower):
        blocks = "reliable Disbursement timestamp reconciliation and disbursement reporting"
